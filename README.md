...
### 5.1 BG-01 设备检修诊断（已实现，可运行）

| 文档 | 说明 |
| --- | --- |
| [环境与启动说明](./docs/环境与启动说明.md) | Python 3.12 环境、**WSL2 向量模型服务必须先启动**、依赖安装、启动自检 |
| [使用与测试指南](./docs/使用与测试指南.md) | 完成度清单、接口用法、回归测试入口与验收要点 |
| [业务场景设计_设备检修诊断](./docs/业务场景设计_设备检修诊断.md) | 场景设计基线（V0.5）：节点契约、结构化结论、实测性能与已知限制 |
| [演示SOP_设备检修诊断](./docs/演示SOP_设备检修诊断.md) | 演示前检查清单、4 个用例脚本、异常处置 |
| [传感器数据资产说明](./docs/传感器数据资产说明.md) | 5 份仿真数据集的场景定义与注入口径（承接 SENSOR-SPEC-001 §4） |
